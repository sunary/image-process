__author__ = 'sunary'


from PIL import Image
import os


class EdgeDetect():

    def __init__(self):
        self.current_dir = os.path.dirname(__file__)

    def read_image(self):
        self.img = Image.open(self.current_dir + '/../resources/face02.jpg')
        data = self.img.getdata()

        self.pix = [[0]*self.img.size[1] for _ in range(self.img.size[0])]
        for i in range(self.img.size[0]):
            for j in range(self.img.size[1]):
                self.pix[i][j] = (data[j*self.img.size[0] + i][0] << 16) | (data[j*self.img.size[0] + i][1] << 8) | (data[j*self.img.size[0] + i][2])

    def save_image(self):
        for i in range(self.img.size[0]):
            for j in range(self.img.size[1]):
                self.img.putpixel((i, j), ((self.pix[i][j] & 0x00ff0000) >> 16, (self.pix[i][j] & 0x0000ff00) >> 8, (self.pix[i][j] & 0x000000ff)))
        self.img.save(self.current_dir + '/../resources/result.png')

    def convert_gray(self):
        self.pix_gray = [[0]*self.img.size[1] for _ in range(self.img.size[0])]
        for i in range(self.img.size[0]):
            for j in range(self.img.size[1]):
                self.pix_gray[i][j] = (((self.pix[i][j] & 0x00ff0000) >> 16) + ((self.pix[i][j] & 0x0000ff00) >> 8) + (self.pix[i][j] & 0x000000ff))/3

    def detect(self):
        temp_x = [[0]*self.img.size[1] for i in range(self.img.size[0])]
        temp_y = [[0]*self.img.size[1] for i in range(self.img.size[0])]

        self._sobel()

        for i in range(1, self.img.size[0] - 1):
            for j in range(1, self.img.size[1] - 1):
                for d1 in range(len(self.gx)):
                    for d2 in range(len(self.gx)):
                        temp_x[i][j] += self.gx[d1][d2]*self.pix_gray[i + d1 - 1][j + d2 -1]
                        temp_y[i][j] += self.gy[d1][d2]*self.pix_gray[i + d1 - 1][j + d2 -1]

        for i in range(0, self.img.size[0] - 0):
            for j in range(0, self.img.size[1] - 0):
                self.pix[i][j] = 0x000000 if (temp_x[i][j]*temp_x[i][j] + temp_y[i][j]*temp_y[i][i] > self.threshold) else 0xffffff

    def _sobel(self):
        self.gx = [[-1, 0, 1],
              [-2, 0, 2],
              [-1, 0, 1]]
        self.gy = [[-1, -2, -1],
              [0, 0, 0],
              [1, 2, 1]]

        self.threshold = 3000

    def _robert(self):
        self.gx = [[1, 0],
              [0, -1]]
        self.gy = [[0, 1],
              [-1, 0]]

        self.threshold = 400

    def _prewitt(self):
        self.gx = [[-1, 0, 1],
              [-1, 0, 1],
              [-1, 0, 1]]
        self.gy = [[-1, -1, -1],
              [0, 0, 0],
              [1, 1, 1]]

        self.threshold = 2000


if __name__ == '__main__':
    edge_detect = EdgeDetect()
    edge_detect.read_image()
    edge_detect.convert_gray()
    edge_detect.detect()
    edge_detect.save_image()